...
# this file is based on squad_dataset.py
# but it is used to read in the final test sets.

# when get_data is called, this class returns 3 test sets:
# add anyplace 4, add one sent, and add best sent

class SquadTestDataset(object):
    dataset_name = 'squad'
    padding = '--PAD--'
    unknown = '--UNK--'
    eos = '--EOS--'
    ...
    def __init__(self, dataset_h5='squad_testset.1.0.h5', data_path='test_data/', ignore_case=False, **kwargs):
        super(SquadTestDataset, self).__init__(**kwargs)
    ...
    def _handle_story_split(self, caches, file_path):
        ...
        story_file = file_path + "-story.txt" #minor change from original file here for clarity (filenames here should be consistent with the actual filenames)
        question_file = file_path + "-question.txt"
        answer_range_file = file_path + "-answer-range.txt"
    ...
    def _handle_story(self):
        ...
        self._handle_story_split(
            cache_train, self.data_path + 'test-any-4')
        self._handle_story_split(
            cache_valid, self.data_path + 'test-add-one-sent')
        self._handle_story_split(
            cache_test, self.data_path + 'test-add-best-sent')
